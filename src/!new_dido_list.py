...
def build_supplier_dict(suppliers: dict) -> dict:
    """ buils a dictionary of suppliers with a list of projects for each
    supplier

    Args:
        suppliers (dict): list of suppliers from config.yaml

    Returns:
        dict: list of supplier with a list of projects for each supplier
    """
    supplier_dict = dict()
    if suppliers is not None and len(suppliers) > 0:
        for supplier_name, rest in suppliers.items():
            project_list = []

            for project_name, rest in suppliers[supplier_name].items():
                project_list.append(project_name)

            supplier_dict[supplier_name] = project_list
...
def add_deliveries_to_projects(deliveries: dict,
                               supplier_dict: dict,
                               strict: bool,
                              ) -> dict:
...
def dido_list(header: str = None):
    ...
    x = build_supplier_dict(leveranciers)
    print(x)
